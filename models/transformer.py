...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed, tgt=None, src_lines=None, mask_lines=None, pos_embed_lines=None):
        ...
        if self.use_lines:
            ...
            memory_lines = src_lines

            hs_line = self.decoder_line(tgt, memory_lines, pos=pos_embed_lines, query_pos=query_embed)

            hs_context = self.decoder(tgt, memory, pos=pos_embed, query_pos=query_embed)

            hs = hs_context + hs_line

            if self.loss_layer_num > 1 and self.loss_layer_num <= 3:
                hs_final = hs[-1]
                hs_c = hs_context[-1]
                hs_l = hs_line[-1]
